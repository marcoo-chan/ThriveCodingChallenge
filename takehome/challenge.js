...
function isArray(array1, array2) {
  if (!Array.isArray(array1) || !Array.isArray(array2)) {
    ...
    console.log(`\nThere are ${array1.length} users and ${array2.length} companies.`);
  }
}
...
function generateOutput(company, activeUsers) {
  ...
  console.log(`\n${company.name} ID: ${company.id}, # of active users: ${activeUsersForCompany.length}`);
  ...
  console.log(`# of users emailed: ${usersEmailed}, # of users not emailed ${usersNotEmailed}`);
  console.log(`Total amount of top ups: ${totalTopUp}`);
  return companyOutput;
}
...
function generateOutputFile (output) {
  try {
    ...
    console.log(`\nOutput.txt file generated successfully.`);
    console.log(`\n===== OUTPUT =====\n ${output}`);
  } catch (error) {
...
function main() {
  ...
  let inactiveUsers = users.length - activeUsers.length;
  console.log(`# of active users: ${activeUsers.length}`);
  console.log(`# of inactive users: ${inactiveUsers}`);
